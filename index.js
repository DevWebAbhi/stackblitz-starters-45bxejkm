...
app.put("/menu/:id",async(req,res)=>{
  try {
    ...
    if(description){
      await menuModel.findOneAndUpdate({id:id},{name,description,price});
    }else{
      await menuModel.findOneAndUpdate({id:id},{name,price});
    }
    return res.status(200).send({msg:"Menu item updated sucessfully"});
  } catch (error) {
    return res.status(500).send({message:"something went wrong",error});
  }
})
...
app.delete("/menu/:id",async(req,res)=>{
  try {
    ...
    return res.status(200).send({msg:"Menu item deleted sucessfully"});
  } catch (error) {
    return res.status(500).send({message:"something went wrong",error});
  }
})
